<x-layout>
    ...
    <div id="wrapper" class="mt-4">
        <form id="form" action="{{ route('materials.update', [app()->getLocale(), $material->id]) }}" method="POST">
            @csrf

            <div id="titleField" class="mb-4">
                <label for="file_name" class="block font-semibold m-2">{{__('materials.name')}}</label>
                <input type="text" id="materialTitle" name="file_name" class="input mt-3" value="{{ old('file_name') ?? $material->file_name}}">
            </div>
                ...
                <div>
                    ...
                    <select name="language_id" id="language" class="input px-2 py-1">
                        @foreach($languages as $language)
                            <option value="{{ $language->id }}" {{ old('language') ?? $material->language_id == $language->id ? 'selected' : '' }}>
                                {{ __($language->id) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="language_level_id" id="level" class="w-full px-2 py-1 input">
                        @foreach($languageLevels as $level)
                            <option value="{{ $level->id }}" {{ old('level') ?? $material->language_level_id == $level->id ? 'selected' : '' }}>{{ $level->id }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="language_aspect_id" id="aspect" class="w-full px-2 py-1 input">
                        @foreach($languageAspects as $aspect)
                            <option value="{{ $aspect->id }}" {{ old('aspect') ?? $material->language_level_id == $aspect->id ? 'selected' : '' }}>
                                {{ __($aspect->id) }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" placeholder="{{__('materials.optDescription')}}" maxlength="500" rows="4" class="w-full input px-2 py-1">{{ old('description') ?? $material->description }}</textarea>
            </div>

            <div class="text-center">
                <button type="submit" class="btn px-4 py-2 input hover:bg-gray-100">{{__('materials.update')}}</button>
            </div>
        </form>
    </div>
</x-layout>
